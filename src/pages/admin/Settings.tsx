...
const SettingsPage = () => {
    ...
    const [primaryColor, setPrimaryColor] = useState("224 60% 55%"); // Default indigo

    const handleApplyBrandColor = (hslValue: string) => {
        setPrimaryColor(hslValue);
        document.documentElement.style.setProperty('--primary', hslValue);
        document.documentElement.style.setProperty('--ring', hslValue);
        document.documentElement.style.setProperty('--glow', hslValue);
        localStorage.setItem('codenest-primary-color', hslValue);
        toast.success("Branding instantly updated across the platform!");
    };
        ...
        <div className="space-y-6">
            ...
            <Tabs defaultValue="profile" className="space-y-6">
                ...
                <TabsContent value="appearance" className="space-y-6">
                    ...
                    <SettingsSection
                        icon={Palette}
                        title="Real-Time Branding Engine 🚀"
                        description="Instantly change the primary color of the entire platform."
                    >
                        <div className="space-y-6">
                            <div className="flex flex-wrap gap-4">
                                {[
                                    { name: "CodeNest Indigo", hsl: "224 60% 55%", bg: "bg-indigo-500" },
                                    { name: "Hacker Emerald", hsl: "150 60% 50%", bg: "bg-emerald-500" },
                                    { name: "Cyberpunk Pink", hsl: "320 80% 55%", bg: "bg-pink-500" },
                                    { name: "Void Violet", hsl: "270 60% 55%", bg: "bg-violet-500" },
                                    { name: "Neon Cyan", hsl: "190 90% 50%", bg: "bg-cyan-500" },
                                    { name: "Solar Orange", hsl: "24 90% 55%", bg: "bg-orange-500" },
                                ].map((preset) => (
                                    <motion.button
                                        key={preset.name}
                                        whileHover={{ scale: 1.05 }}
                                        whileTap={{ scale: 0.95 }}
                                        onClick={() => handleApplyBrandColor(preset.hsl)}
                                        className={cn(
                                            "group relative flex items-center gap-3 rounded-full border border-border/50 bg-card p-2 pr-4 transition-colors hover:border-primary/50",
                                            primaryColor === preset.hsl && "border-primary bg-primary/10 shadow-[0_0_15px_rgba(var(--primary),0.3)]"
                                        )}
                                    >
                                        <div className={cn("h-8 w-8 rounded-full shadow-inner border border-white/20", preset.bg)} />
                                        <span className="text-sm font-semibold">{preset.name}</span>
                                        {primaryColor === preset.hsl && (
                                            <Check className="h-4 w-4 text-primary absolute right-2 opacity-50" />
                                        )}
                                    </motion.button>
                                ))}
                            </div>
                            ...
                            <div className="space-y-2 pt-4 border-t border-border/50">
                                <Label htmlFor="custom-hsl">Custom HSL Value</Label>
                                <div className="flex gap-3 max-w-sm">
                                    <Input
                                        id="custom-hsl"
                                        value={primaryColor}
                                        onChange={(e) => setPrimaryColor(e.target.value)}
                                        placeholder="e.g. 224 60% 55%"
                                        className="font-mono text-sm"
                                    />
                                    <Button onClick={() => handleApplyBrandColor(primaryColor)} variant="secondary">Apply Custom</Button>
                                </div>
                                <p className="text-xs text-muted-foreground mt-2">
                                    Note: Changes are applied instantly to your local session. To persist across all users, you would typically save this to the `site_settings` database table.
                                </p>
                            </div>
                        </div>
                    </SettingsSection>
